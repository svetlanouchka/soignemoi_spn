...
// Requête SQL pour récupérer les patients qui finissent leur séjour aujourd'hui
$query = $pdo->prepare("SELECT u.id AS user_id, u.prenom, u.nom
                        FROM sejours s
                        JOIN user u ON s.user_id = u.id
                        WHERE s.date_fin = :date_today");
$query->bindParam(':date_today', $date_today, PDO::PARAM_STR);
$query->execute();
$patientsEndToday = $query->fetchAll(PDO::FETCH_ASSOC);

// Retourner les patients en JSON
echo json_encode($patientsEndToday);
?>
